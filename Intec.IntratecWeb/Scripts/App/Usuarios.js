function IniciarSesion() {
    _peticionAjax_Promise(URL_SERVICE + URI_SERVICE.LogIn, VerbosREST.POST,
        JSON.stringify({ NumeroIdentificacion: $('#NumeroIdentificacion_LogIn').val(), Password: $('#Password_LogIn').val() }))
        .then(response => {                   
            if (response.data != null) {
                _peticionAjax('../Session/AddUsuarioSesion', VerbosREST.POST, JSON.stringify({ UsuarioSesion: response.data }), false);
